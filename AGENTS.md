...
- LLM SDK（`@earendil-works/pi-ai`）只允许出现在 `src/providers/`；`@earendil-works/pi-coding-agent`（发现 Agent）只允许出现在 `src/discovery/`；其余代码只依赖 `LlmProvider` 协议与 `DiscoverySession` 接口——这是供应商可替换性的边界，违反即架构回退。

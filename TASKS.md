...
**阶段闸门 3**：⏳ 第二轮 24h 观测进行中——**2026-09-18 09:57** 重启起算（`--log-file data/daemon.log` 文件日志），预计 **2026-09-19 09:57** 复核。
- 第一轮观测（09-17 00:33 起）**未能跑满 24h**：进程于当日 22:35 被宿主机外部事件中断（`last` 证据：22:35 有终端会话变动、次日 07:34 主机重启），非代码缺陷——中断前日志 0 错误、台账全部健康（SKIPPED_* 正常入账，MiniMax 配额耗尽期间优雅降级）；但按"连续 24h"标准此窗口不作数
- 教训已修：daemon 日志原先走会话管道（进程异常终止即丢失），现支持 `--log-file` 落盘 `data/daemon.log`，中断可追因
- 已验证项（两轮累积）：退避与恢复（确定性单测 + MiniMax 配额耗尽真实演练）、供应商降级（集成测试）、预算熔断、旧守护进程 SIGINT 优雅退出（两轮重启均干净排干）
- 复核指标：进程存活、台账行数 = 调度次数、`data/daemon.log` 无未处理异常
...
**阶段闸门 5**：⏳ 与闸门 3 同窗口复核（2026-09-19 09:57）——面板并发只读 + 配置变更可追溯。已验证：面板 headless 冒烟、查询层单测、tick 配置变更即时生效单测、webapp 冒烟期间与 daemon 并发只读互不干扰；复核时确认 24h 内持续可用。
